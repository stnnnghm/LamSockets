...
// NewMongoStrore creates a MongoStore
...
const (
	connectionsColName = "connections"
	connActive = "active"
	connClosed = "closed"
)
...
type connectionIDEntry struct {
	...
	DisconnnectTs time.Time
	ConnectionID string
	Status string
}

func (store *MongoStore) readConnections(ctx context.Context) ([]string, error) {
	collection := store.db.Collection(connectionsColName)
	filter := bson.M{"status": bson.M{"$eq": "active"}}
	cur, err := collection.Find(context.TODO(), filter)
	if err != nil {
		log.Println("Error while opening the curson on the connection collection", err.Error())
		return nil, err
	}
